...
export function Pitch({ formation, squad, assignments, onSlotClick }: PitchProps) {
  return (
    <div className="pitch-stripes relative mx-auto mt-4 aspect-[3/4] w-full max-w-sm flex-1 overflow-hidden rounded-3xl border border-white/10 bg-turf shadow-inner">
      ...
      {formation.slots.map((slot) => {
        const { x, y } = slotCoordinates(formation, slot.id)
        const playerId = assignments[slot.id]
        const player = playerId ? squad.players.find((p) => p.id === playerId) : undefined
        ...
        return (
          ...
          >
            <motion.span
              ...
              className={`flex h-12 w-12 items-center justify-center rounded-full border-2 font-display text-base shadow-lg ${
                player
            ...
            <span className="max-w-[4.5rem] truncate font-display text-[11px] tracking-wide text-chalk/90">
              {player ? player.name.split(' ').at(-1) : slot.position}
            </span>
          </button>
        )
      })}
